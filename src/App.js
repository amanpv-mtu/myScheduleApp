...
function App() {
  ...
  }, [pomodoroTimer.running, pomodoroTimer.timeLeft, pomodoroTimer.mode, pomodoroSettings, currentPomodoroBlockId, dailyScheduleState, blockTimeConsumed]); // Added new dependencies
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 font-inter text-gray-800 flex flex-col items-center">
      <div className="w-full max-w-4xl bg-white shadow-xl rounded-xl p-6 mb-8">
        {/* Header and Tabs */}
        ...
        {activeTab === 'schedule' && (
          <div>
            ...
            {/* Pomodoro Timer */}
            <div className="bg-indigo-50 rounded-lg p-4 mb-6 text-center shadow-inner">
              ...
              {nextTask && (
                <p className="text-base text-indigo-700 font-semibold mt-4 flex items-center justify-center">
                  <FastForward size={18} className="mr-2" /> Next: {nextTask.activity} ({nextTask.plannedStart} - {nextTask.plannedEnd})
                </p>
              )}
            </div>
